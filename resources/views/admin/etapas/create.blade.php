...
<div class="container mx-auto py-8 px-32"> <!-- Doble margen lateral -->
    <div class="relative mb-6">
        ...
        <h1 class="text-2xl font-bold text-center">Agregar Fase</h1>
    </div>
    <div class="p-6 w-full"> <!-- Formulario extendido a toda la pantalla -->
        <form action="{{ route('admin.phases.store') }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <input type="text" name="name" id="name" maxlength="20" class="w-full border border-gray-300 rounded px-3 py-2 focus:outline-none focus:ring focus:border-blue-400" value="{{ old('name') }}" required>
                @error('name')<span class="text-red-500 text-sm">{{ $message }}</span>@enderror
            </div>
            <div class="mb-4">
                <label for="description" class="block text-gray-700 font-semibold mb-2">Descripción</label>
                <textarea name="description" id="description" rows="3" maxlength="30" class="w-full border border-gray-300 rounded px-3 py-2 focus:outline-none focus:ring focus:border-blue-400">{{ old('description') }}</textarea>
