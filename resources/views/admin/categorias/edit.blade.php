...
<div class="min-h-[80vh] bg-gray-100 px-6 py-10">
  <h1 class="text-3xl font-semibold text-center text-slate-700 mb-8">Editar Categoría</h1>
  ...
  <div class="bg-[#F1F1F1] rounded-2xl shadow-lg p-10 w-full">
    <form action="{{ route('admin.categorias.update', $categoria->id) }}" method="POST" class="space-y-6">
      ...
      <div>
        ...
        <input type="text" name="nombre" id="nombre-input" value="{{ old('nombre', $categoria->nombre) }}" maxlength="30" class="w-full rounded-md border border-gray-300 px-4 py-2 focus:border-blue-900 focus:ring-2 focus:ring-blue-100 outline-none @error('nombre') border-red-500 @enderror" placeholder="Nombre de Categoría" required>
        @error('nombre')
      ...
      <div>
        ...
        <textarea name="descripcion" rows="2" class="w-full rounded-md border border-gray-300 px-4 py-2 focus:border-blue-900 focus:ring-2 focus:ring-blue-100 outline-none" placeholder="Descripción de la categoría" required>{{ old('descripcion', $categoria->descripcion) }}</textarea>
      </div>
      <div>
      ...
      <div class="flex justify-center gap-8 mt-8">
        ...
        <button type="submit" id="submit-btn" class="bg-[#0B2049] hover:brightness-110 text-white font-semibold rounded-full px-8 py-3">Actualizar</button>
      </div>
    </form>
  </div>
</div>
...
<script>
  ...
  document.addEventListener('DOMContentLoaded', function() {
    ...
    // Cargar niveles existentes desde la base de datos
    @if(isset($categoria))
      @foreach(['primero', 'segundo', 'tercero', 'cuarto', 'quinto', 'sexto'] as $nivel)
        @if($categoria->{$nivel})
