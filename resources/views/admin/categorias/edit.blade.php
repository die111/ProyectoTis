  ...
  <div class="bg-[#F1F1F1] rounded-2xl shadow-lg p-10 w-full max-w-2xl">
    <form action="{{ route('admin.categorias.update', $categoria->id) }}" method="POST" class="space-y-6">
      ...
      <div>
        ...
        <input type="text" name="nombre" value="{{ old('nombre', $categoria->nombre) }}" class="w-full rounded-md border border-gray-300 px-4 py-2 focus:border-blue-900 focus:ring-2 focus:ring-blue-100 outline-none" placeholder="Responsable de Área" required>
      </div>
      <div>
        <label class="block text-sm font-medium text-gray-700 mb-1">Descripción:</label>
        <textarea name="descripcion" rows="2" class="w-full rounded-md border border-gray-300 px-4 py-2 focus:border-blue-900 focus:ring-2 focus:ring-blue-100 outline-none" placeholder="Se encarga de administrar una área en específico" required>{{ old('descripcion', $categoria->descripcion) }}</textarea>
      </div>
      <div>
