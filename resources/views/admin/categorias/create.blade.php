  ...
  <div class="bg-[#F1F1F1] rounded-2xl shadow-lg p-10 w-full">
    <form action="{{ route('admin.categorias.store') }}" method="POST" class="space-y-6">
      ...
      <div>
        ...
        <input type="text" name="nombre" id="nombre-input" value="{{ old('nombre') }}" maxlength="30" class="w-full rounded-md border border-gray-300 px-4 py-2 focus:border-blue-900 focus:ring-2 focus:ring-blue-100 outline-none @error('nombre') border-red-500 @enderror" placeholder="Nombre de Categoría" required>
        @error('nombre')
