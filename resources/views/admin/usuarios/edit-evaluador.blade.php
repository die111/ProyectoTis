...
@section('content')
  <div class="container mx-auto py-10">
    <div class="max-w-4xl mx-auto bg-white rounded-2xl shadow-lg overflow-hidden">
      ...
      <div class="bg-[#0C3E92] text-white px-6 py-5 flex items-center justify-between">
        <h2 class="text-2xl font-semibold">Editar Evaluador</h2>
      </div>
          ...
          <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
            ...
            <div>
              ...
              <input id="last_name_father" type="text" name="last_name_father" required
                placeholder="Ingrese su apellido paterno" value="{{ old('last_name_father', $user->last_name_father) }}"
                class="w-full rounded-xl border border-gray-300 bg-white px-4 py-3 shadow-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 placeholder:text-gray-400">
            </div>
              ...
              <select id="area_id" name="area_id" required
                ...
                @foreach($areas as $area)
                  @if($area->is_active || $user->area_id == $area->id)
                    <option value="{{ $area->id }}" {{ $user->area_id == $area->id ? 'selected' : '' }}>{{ $area->name }}
                    </option>
                  @endif
                @endforeach
              </select>
            ...
            <div class="relative">
              <label for="password_edit_eval" class="block text-sm font-semibold text-gray-800 mb-1">Contraseña*</label>
              <input id="password_edit_eval" name="password" type="password" required minlength="8"
              ...
              <button type="button" id="togglePasswordEditEval"
                ...
                <!-- ojo -->
                <svg id="icon-eye-edit-eval" xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 24 24"
                  fill="none" stroke="currentColor" stroke-width="2">
                  <path d="M2 12s3.5-7 10-7 10 7 10 7-3.5 7-10 7-10-7-10-7Z" />
                  <circle cx="12" cy="12" r="3" />
                </svg>
                <!-- ojo tachado -->
                <svg id="icon-eye-off-edit-eval" xmlns="http://www.w3.org/2000/svg" class="hidden h-5 w-5"
              ...
              <small class="text-gray-500 block mt-1">Dejar en blanco para no cambiar</small>
            </div>
            ...
            <div class="relative">
              ...
              <select id="level" name="level"
                ...
                <option value="Primaria" {{ $user->level == 'Primaria' ? 'selected' : '' }}>Primaria</option>
                <option value="Secundaria" {{ $user->level == 'Secundaria' ? 'selected' : '' }}>Secundaria</option>
                <option value="Preuniversitario" {{ $user->level == 'Preuniversitario' ? 'selected' : '' }}>Preuniversitario
                </option>
              </select>
              <i class="bi bi-chevron-down absolute right-3 top-9 pointer-events-none text-gray-500"></i>
            </div>
  ...
  <!-- Toggle contraseña (IDs únicos para esta vista) -->
  <script>
    (function () {
      const input = document.getElementById('password_edit_eval');
      const btn = document.getElementById('togglePasswordEditEval');
      const eye = document.getElementById('icon-eye-edit-eval');
      const eyeOff = document.getElementById('icon-eye-off-edit-eval');
