    ...
    <div class="-mx-4 lg:-mx-6 w-full">
        <div class="px-4 lg:px-6">
            <div class="mb-8 flex items-center justify-between">
                <div>
                    <h1 class="text-3xl font-semibold text-gray-900 mb-2">Crear Rol</h1>
                    <p class="text-gray-600">Crea un nuevo rol y asigna los permisos correspondientes.</p>
                </div>
            ...
            <form action="{{ route('admin.roles.store') }}" method="POST">
                ...
                <div class="bg-white rounded-lg border border-gray-200 shadow-sm mb-6">
                    ...
                    <div class="p-6 space-y-4">
                        <div>
                            ...
                            <input type="text" name="name" id="name" maxlength="20" class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500" value="{{ old('name') }}" required>
                            @error('name')<span class="text-red-500 text-sm">{{ $message }}</span>@enderror
                        </div>

                        <div>
                            <label for="description" class="block text-sm font-medium text-gray-700 mb-2">Descripción</label>
                            <textarea name="description" id="description" rows="3" maxlength="30" class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500">{{ old('description') }}</textarea>
                ...
                <div class="flex justify-end gap-3">
                    ...
                    <button type="submit" class="btn btn-primary bg-[#091c47] text-white px-6 py-2 rounded-md">Crear Rol</button>
                </div>
            </form>
        </div>
    </div>
